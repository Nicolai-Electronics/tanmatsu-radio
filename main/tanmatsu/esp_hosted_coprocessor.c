...
static inline esp_err_t populate_buff_handle(interface_buffer_handle_t* buf_handle, uint8_t if_type, uint8_t* buf,
                                             uint16_t len, void (*free_buf_func)(void* data), void* free_buf_handle,
                                             uint8_t flag, uint8_t if_num, uint16_t seq_num) {
...
#endif
            ESP_HEXLOGW("Wakeup_pkt", buf_handle->payload + H_ESP_PAYLOAD_HEADER_OFFSET, buf_handle->payload_len,
                        wakeup_pkt_display_len);
...
static int host_to_slave_reconfig(uint8_t* evt_buf, uint16_t len) {
    ...
    while (len_left) {
        ...
        if (*pos == HOST_CAPABILITIES) {
            ...
            ESP_LOGI(TAG, "ESP<-Host wifi flow ctl start thres [%u%%]", slv_cfg_g.throttle_high_threshold);

            /* Warn if FreeRTOS tick is small */
            if ((slv_cfg_g.throttle_low_threshold > 0) && (CONFIG_FREERTOS_HZ < 1000)) {
                ESP_LOGW(TAG,
                         "FreeRTOS tick[%d]<1000. Enabling flow control with lower FrerRTOS tick may result in lower "
            ...
            ESP_LOGI(TAG, "ESP<-Host wifi flow ctl clear thres [%u%%]", slv_cfg_g.throttle_low_threshold);
...
static void process_priv_pkt(uint8_t* payload, uint16_t payload_len) {
    ...
    if (event->event_type == ESP_PRIV_EVENT_INIT) {
        ...
        lora_protocol_handle_packet(event->event_data, event->event_len);
...
int event_handler(uint8_t val) {
    switch (val) {
        ...
        case ESP_POWER_SAVE_ON:
            xTaskCreate(power_save_alert_task, "ps_alert_task", 3072, (void*)ESP_POWER_SAVE_ON, tskIDLE_PRIORITY + 5,
                        NULL);
            break;
        ...
        case ESP_POWER_SAVE_OFF:
            ...
            xTaskCreate(power_save_alert_task, "ps_alert_task", 3072, (void*)ESP_POWER_SAVE_OFF, tskIDLE_PRIORITY + 5,
                        NULL);
            break;
    }
    return 0;
...
static void register_reset_pin(uint32_t gpio_num) {
    if (gpio_num != -1) {
        ...
        gpio_config_t slave_reset_pin_conf = {
            .intr_type = GPIO_INTR_DISABLE, .mode = GPIO_MODE_INPUT, .pull_up_en = 1, .pin_bit_mask = (1 << gpio_num)};
...
static int connect_sta(void) {
    ...
    if (country_code_has_set == false) {
        wifi_country_t country = {.cc = "CN", .schan = 1, .nchan = 13, .policy = 0};
...
esp_err_t esp_hosted_coprocessor_init(void) {
    ...
    assert(xTaskCreate(recv_task, "recv_task", CONFIG_ESP_HOSTED_DEFAULT_TASK_STACK_SIZE, NULL,
    ...
    assert(xTaskCreate(send_task, "send_task", CONFIG_ESP_HOSTED_DEFAULT_TASK_STACK_SIZE, NULL,
                       CONFIG_ESP_HOSTED_DEFAULT_TASK_PRIORITY, NULL) == pdTRUE);
#endif
    ...
    assert(xTaskCreate(host_reset_task, "host_reset_task", CONFIG_ESP_HOSTED_DEFAULT_TASK_STACK_SIZE, NULL,
                       CONFIG_ESP_HOSTED_DEFAULT_TASK_PRIORITY, NULL) == pdTRUE);
...
#define I2C_COPROC_REG_MESSAGE 146

esp_err_t i2c_set_message_led(i2c_master_dev_handle_t handle, bool red, bool green, bool blue, bool red_b, bool green_b,
                              bool blue_b, bool fade, bool fade_hold) {
    uint8_t data = (red << 0) | (green << 1) | (blue << 2) | (fade << 3) | (red_b << 4) | (green_b << 5) |
                   (blue_b << 6) | (fade_hold << 7);
    uint8_t packet[2] = {I2C_COPROC_REG_MESSAGE, data};
    return i2c_master_transmit(handle, packet, sizeof(packet), I2C_TIMEOUT_MS);
}
...
void i2c_master_test(void) {
    ...
    while (1) {
        while (i2c_ll_is_bus_busy(I2C_LL_GET_HW(LP_I2C_NUM_0))) {
            printf(".");
            vTaskDelay(pdMS_TO_TICKS(10));
        }
        esp_err_t res = i2c_set_message_led(dev_handle, true, false, false, false, false, false, false, false);
        if (res == ESP_OK) {
            ESP_LOGI(TAG, "LED RED ON");
        } else {
            ESP_LOGE(TAG, "Failed to set LED RED ON");
        }
        ...
        res = i2c_set_message_led(dev_handle, false, true, false, false, false, false, false, false);
        if (res == ESP_OK) {
            ESP_LOGI(TAG, "LED GREEN ON");
        } else {
            ESP_LOGE(TAG, "Failed to set LED GREEN ON");
        }
        vTaskDelay(pdMS_TO_TICKS(500));
    }
}
...
void app_main(void) {
    ...
    ESP_ERROR_CHECK(spi_initialize());
