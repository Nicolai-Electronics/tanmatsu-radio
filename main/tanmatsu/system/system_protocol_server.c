...
static void system_protocol_appfs_list(uint32_t sequence_number, const uint8_t* payload, size_t payload_length) {
    if (payload_length < sizeof(system_protocol_nvs_location_t)) {
        ESP_LOGW(TAG, "NVS list command received with insufficient data length: %zu bytes", payload_length);
        system_protocol_send_nack(sequence_number);
        return;
...
static void system_protocol_appfs_read(uint32_t sequence_number, const uint8_t* payload, size_t payload_length) {
    if (payload_length < sizeof(system_protocol_nvs_location_t)) {
        ESP_LOGW(TAG, "NVS list command received with insufficient data length: %zu bytes", payload_length);
        system_protocol_send_nack(sequence_number);
        return;
...
static void system_protocol_appfs_write(uint32_t sequence_number, const uint8_t* payload, size_t payload_length) {
    if (payload_length < sizeof(system_protocol_nvs_location_t)) {
        ESP_LOGW(TAG, "NVS list command received with insufficient data length: %zu bytes", payload_length);
        system_protocol_send_nack(sequence_number);
        return;
...
static void system_protocol_appfs_delete(uint32_t sequence_number, const uint8_t* payload, size_t payload_length) {
    if (payload_length < sizeof(system_protocol_nvs_location_t)) {
        ESP_LOGW(TAG, "NVS list command received with insufficient data length: %zu bytes", payload_length);
        system_protocol_send_nack(sequence_number);
        return;
...
static void system_protocol_appfs_checksum(uint32_t sequence_number, const uint8_t* payload, size_t payload_length) {
    if (payload_length < sizeof(system_protocol_nvs_location_t)) {
        ESP_LOGW(TAG, "NVS list command received with insufficient data length: %zu bytes", payload_length);
        system_protocol_send_nack(sequence_number);
        return;
...
static void system_protocol_appfs_get_usage(uint32_t sequence_number, const uint8_t* payload, size_t payload_length) {
    if (payload_length < sizeof(system_protocol_nvs_location_t)) {
        ESP_LOGW(TAG, "NVS list command received with insufficient data length: %zu bytes", payload_length);
        system_protocol_send_nack(sequence_number);
        return;
...
static void system_protocol_appfs_boot(uint32_t sequence_number, const uint8_t* payload, size_t payload_length) {
    if (payload_length < sizeof(system_protocol_nvs_location_t)) {
        ESP_LOGW(TAG, "NVS list command received with insufficient data length: %zu bytes", payload_length);
        system_protocol_send_nack(sequence_number);
        return;
...
static void system_protocol_packet_callback(uint32_t msg_id, const uint8_t* request_buffer, size_t request_length) {
    ...
    switch (packet->type) {
        ...
        case SYSTEM_PROTOCOL_TYPE_NVS_DELETE:
            ...
            break;
        case SYSTEM_PROTOCOL_TYPE_APPFS_LIST:
            system_protocol_appfs_list(packet->sequence_number, payload, payload_length);
            break;
        case SYSTEM_PROTOCOL_TYPE_APPFS_READ:
            system_protocol_appfs_read(packet->sequence_number, payload, payload_length);
            break;
        case SYSTEM_PROTOCOL_TYPE_APPFS_WRITE:
            system_protocol_appfs_write(packet->sequence_number, payload, payload_length);
            break;
        case SYSTEM_PROTOCOL_TYPE_APPFS_DELETE:
            system_protocol_appfs_delete(packet->sequence_number, payload, payload_length);
            break;
        case SYSTEM_PROTOCOL_TYPE_APPFS_CHECKSUM:
            system_protocol_appfs_checksum(packet->sequence_number, payload, payload_length);
            break;
        case SYSTEM_PROTOCOL_TYPE_APPFS_GET_USAGE:
            system_protocol_appfs_get_usage(packet->sequence_number, payload, payload_length);
            break;
        case SYSTEM_PROTOCOL_TYPE_APPFS_BOOT:
            system_protocol_appfs_boot(packet->sequence_number, payload, payload_length);
            break;
        case SYSTEM_PROTOCOL_TYPE_FS_LIST:
            system_protocol_fs_list(packet->sequence_number, payload, payload_length);
            break;
        case SYSTEM_PROTOCOL_TYPE_FS_READ:
            system_protocol_fs_read(packet->sequence_number, payload, payload_length);
            break;
        case SYSTEM_PROTOCOL_TYPE_FS_WRITE:
            system_protocol_fs_write(packet->sequence_number, payload, payload_length);
            break;
        case SYSTEM_PROTOCOL_TYPE_FS_DELETE:
            system_protocol_fs_delete(packet->sequence_number, payload, payload_length);
            break;
        case SYSTEM_PROTOCOL_TYPE_FS_CHECKSUM:
            system_protocol_fs_checksum(packet->sequence_number, payload, payload_length);
            break;
        case SYSTEM_PROTOCOL_TYPE_FS_STAT:
            system_protocol_fs_stat(packet->sequence_number, payload, payload_length);
            break;
        case SYSTEM_PROTOCOL_TYPE_FS_MKDIR:
            system_protocol_fs_mkdir(packet->sequence_number, payload, payload_length);
            break;
        case SYSTEM_PROTOCOL_TYPE_FS_RMDIR:
            system_protocol_fs_rmdir(packet->sequence_number, payload, payload_length);
            break;
        case SYSTEM_PROTOCOL_TYPE_FS_GET_USAGE:
            system_protocol_fs_get_usage(packet->sequence_number, payload, payload_length);
            break;
        case SYSTEM_PROTOCOL_TYPE_FS_COPY:
            system_protocol_fs_copy(packet->sequence_number, payload, payload_length);
            break;
        case SYSTEM_PROTOCOL_TYPE_FS_RENAME:
            system_protocol_fs_rename(packet->sequence_number, payload, payload_length);
            break;
        default:
